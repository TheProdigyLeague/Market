...
// --- New Logic for Job Opportunities Section: Monkey Error Modal ---
    ...
    // 2. Define the Handler Function for CARDS (Monkey Modal)
    function showJobEatenModal(event) {
        // Prevent the card/link from navigating/scrolling
        event.preventDefault(); 
        if (jobEatenModal) {
    ...
    // A. Attach Monkey Modal to the three category CARDS
    [softwareCard, dataCard, growthCard].forEach(element => {
        if (element) {
            element.addEventListener('click', showJobEatenModal);
        }
    });

    // B. Attach Mock Redirect to the main "See All Open Roles" BUTTON
    if (seeAllBtn) {
        // *** CRITICAL CHANGE HERE ***
        seeAllBtn.addEventListener('click', injectSketchyBrokerSite);
    }
    
    // 4. Close Modal Logic (remains the same)
    if (closeEatenBtn) {
    ...
    // --- End New Logic ---
// --- New Logic for Footer Navigation Modals ---

    // 1. Get Elements
    const linkAbout = document.getElementById('link-about');
    const linkIR = document.getElementById('link-ir');
    ...
    // Helper to set up modal listeners
    function setupModal(linkElement, modalElement) {
        if (linkElement && modalElement) {
            ...
            // Get the close button and set click listener
    ...
    // 2. Attach Listeners to All Three New Modals
    setupModal(linkAbout, aboutModal);
    setupModal(linkIR, irModal);
    setupModal(linkPrivacy, privacyModal);

    // Note: The Careers (GitHub) and FAQ/Support (Gallery) links are direct links
    // and do not need JavaScript event listeners since they use the target="_blank" attribute.

    // --- End New Logic for Footer Navigation Modals ---
/**
...
/**
 * Injects a new, mock job site page into the current window 
 * when the "See All Open Roles" button is clicked.
 */
function injectSketchyBrokerSite() {
    const sketchyBrokerHTML = `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>🚨 WORK NOW! 🚨 Global Talent Brokerage</title>
    <style>
        body { font-family: monospace; background-color: #f0f0f0; color: #333; padding: 20px; line-height: 1.6; }
        .broker-container { max-width: 800px; margin: 0 auto; background: white; padding: 30px; border: 5px dashed red; box-shadow: 0 0 15px rgba(255,0,0,0.5); }
        h1 { color: red; text-align: center; font-size: 2.5em; margin-bottom: 20px; text-decoration: underline wavy yellow; }
        .warning-banner { background-color: yellow; color: black; padding: 10px; text-align: center; font-weight: bold; margin-bottom: 20px; border: 1px solid black; }
        .job-broker { border: 1px solid #ccc; padding: 15px; margin-bottom: 15px; background-color: #fff8e1; }
        .job-broker h3 { color: #cc0000; margin-top: 0; }
        .job-broker p { font-size: 0.9em; }
        .apply-btn { background-color: #000; color: yellow; padding: 8px 15px; border: none; cursor: pointer; display: block; width: 100%; margin-top: 10px; }
        
        /* Mock Ad Placement */
        .mock-ad { background-color: #333; color: white; padding: 10px; margin-top: 20px; text-align: center; font-size: 0.8em; border: 2px solid green; }
    </style>
</head>
<body>
    <div class="broker-container">
        <h1>🔥🔥 URGENT HIRING NEEDS! APPLY TODAY! 🔥🔥</h1>
        <div class="warning-banner">
            ⚠️ WARNING: Your profile has been flagged as "HIGHLY MOTIVATED". Brokerage fees may apply!
        </div>

        <div class="job-broker">
            <h3>Remote "Data Entry Specialist" (High Volume)</h3>
            <p><strong>Pay:</strong> $5/hour or 0.000001 BTC per data point (whichever is lower)</p>
            <p><strong>Location:</strong> Any jurisdiction with lax labor laws.</p>
            <p><strong>Job Description:</strong> You will be given high-value, poorly-formatted data from third-party sources and asked to "optimize" it. Must work at least 80 hours a week. No benefits.</p>
            <button class="apply-btn">APPLY NOW! START IMMEDIATELY!</button>
        </div>

        <div class="job-broker">
            <h3>"Growth Hacker" (We just need someone who knows SQL)</h3>
            <p><strong>Pay:</strong> Equity in a pre-seed startup that will never launch.</p>
            <p><strong>Location:</strong> Your own desk.</p>
            <p><strong>Job Description:</strong> Title sounds cool, but you'll mostly be scraping competitor websites and sending spam emails. Must be aggressive and morally flexible.</p>
            <button class="apply-btn">APPLY NOW! START IMMEDIATELY!</button>
        </div>

        <div class="mock-ad">
            [Ad] CLICK HERE to get rich quick with our Crypto Pump-and-Dump Guide! (Brought to you by Asset Productions)
        </div>
    </div>
    <script>
        // Simple mock click handler for the new site
        document.querySelectorAll('.apply-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                alert("Application Sent! Check your email for further steps (and a small invoice).");
            });
        });
    </script>
</body>
    ...
    // Overwrite the entire content of the current window
    document.open();
    document.write(sketchyBrokerHTML);
    document.close();
}